...
namespace WebBanDT.Models
{
    public class SamSungPhone
    {
        ...
        public SelectList SamSungPhoneList { get; set; }
        public List<SamSungPhone> GetHome()
        {
            List<SamSungPhone> strList = new List<SamSungPhone>();
            ...
            string sql1 = @"select Phone.Id as 'maDT', Phone.Name as 'PhoneName', Phone.[Image], Manufacturer.[Name] as 'NameNSX',Price, [Status], [configuration] from Phone,Manufacturer where Phone.Manufacturerid = Manufacturer.Id and Manufacturer.Name ='SamSung'";
            SqlDataAdapter cmd = new SqlDataAdapter(sql1, con);
            DataTable dt = new DataTable();
            con.Open();
            cmd.Fill(dt);
            con.Close();
            SamSungPhone strHome;
            for (int i = 0; i < dt.Rows.Count; i++)
            {

                strHome = new SamSungPhone();
